...
void do_io()
{
	...
	int q_count = 0;
	QUEUE_NODE *io_point;
	io_point = (wait_q->front);
	message msg;

	for (q_count = 0; q_count<queueCount(wait_q); q_count++)
	{

		if ((io_point->data->io_burst)>0)
		{
			(io_point->data->io_burst)--;

			if (queueCount(wait_q)>1)
		...
		else
		{
			...
			if (ret == -1) {
				...
				*msgq_id = ret;
				...
				(io_point->data->io_burst) = msg.time_quantum;
