...
void time_tick(int signo)
{
	...
	if (time_quantum>0) {
		...
		//fprintf(output_file,"send signal to %d, remain time quantum : %d\n", run_q->front->pid, time_quantum);
	}
	else {
...
//	fp = fopen("./schedule.txt","a");
	...
	fprintf(output_file,"my pid: %d. cpu_burst: %d\n", getpid(), *cpu_bust);	
...
void child_process(){
	...
	*io_bust = rand() % 10;
	...
	//fprintf(output_file,"child process id : %d, cpu_bust : %d, io_bust : %d\n", getpid(), *cpu_bust, *io_bust);
	execlp(output_file,"child process id : %d, cpu_bust : %d, io_bust : %d\n", getpid(), *cpu_bust, *io_bust);
...
int main(int argc, char* argv) {
	...
	while (runProcess < TOTALFORK) {
		...
		if (pids[runProcess] < 0) {
			...
			fprintf(output_file,"parent %ld, child %ld\n", (long)getpid(), (long)pids[runProcess]);
